...
in_addr CUDPReaderWriter::lookup(const std::string& hostname)
{
	...
	lprint("Cannot find address for host %s", hostname.c_str());
...
bool CUDPReaderWriter::open()
{
	...
	if (m_fd < 0) {
		lprint("Cannot create the UDP socket, err: %d", errno);
		return false;
	}
	...
	if (m_port > 0U) {
		...
		if (m_address.size()) {
			...
			if (addr.sin_addr.s_addr == INADDR_NONE) {
				lprint("The address is invalid - %s", m_address.c_str());
				return false;
			}
		}
		...
		if (::setsockopt(m_fd, SOL_SOCKET, SO_REUSEADDR, (char *)&reuse, sizeof(reuse)) == -1) {
			lprint("Cannot set the UDP socket option (port: %u), err: %d", m_port, errno);
			return false;
		}

		if (::bind(m_fd, (sockaddr*)&addr, sizeof(sockaddr_in)) == -1) {
			lprint("Cannot bind the UDP address (port: %u), err: %d", m_port, errno);
			return false;
		}
	}
...
int CUDPReaderWriter::read(unsigned char* buffer, unsigned int length, in_addr& address, unsigned int& port)
{
	...
	if (ret < 0) {
		lprint("Error returned from UDP select (port: %u), err: %d", m_port, errno);
		return -1;
	}
	...
	if (len <= 0) {
		lprint("Error returned from recvfrom (port: %u), err: %d", m_port, errno);
		return -1;
	}
...
bool CUDPReaderWriter::write(const unsigned char* buffer, unsigned int length, const in_addr& address, unsigned int port)
{
	...
	if (ret < 0) {
		lprint("Error returned from sendto (port: %u), err: %d", m_port, errno);
		return false;
	}
