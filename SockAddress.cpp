...
CSockAddress::CSockAddress()
{
    Clear();
}

CSockAddress::CSockAddress(const struct sockaddr_storage &from)
{
    Clear();
...
CSockAddress::CSockAddress(const int family, const unsigned short port, const char *address)
{
    Clear();
...
void CSockAddress::Initialize(int family, uint16_t port, const char *address)
{
    Clear();
...
CSockAddress &CSockAddress::operator=(const CSockAddress &from)
{
    Clear();
...
bool CSockAddress::operator==(CSockAddress &from)
{
...
bool CSockAddress::AddressIsZero()
{
    if (AF_INET == addr.ss_family) {
        struct sockaddr_in *addr4 = (struct sockaddr_in *)&addr;
...
void CSockAddress::ClearAddress()
{
    if (AF_INET == addr.ss_family) {
        struct sockaddr_in *addr4 = (struct sockaddr_in *)&addr;
...
const char *CSockAddress::GetAddress()
{
    if (AF_INET == addr.ss_family) {
        struct sockaddr_in *addr4 = (struct sockaddr_in *)&addr;
...
unsigned short CSockAddress::GetPort()
{
    if (AF_INET == addr.ss_family) {
        struct sockaddr_in *addr4 = (struct sockaddr_in *)&addr;
...
struct sockaddr *CSockAddress::GetPointer()
{
    return (struct sockaddr *)&addr;
}

size_t CSockAddress::GetSize()
{
    if (AF_INET == addr.ss_family)
