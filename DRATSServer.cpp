...
m_writeLength(0U)
{
	...
	m_writeText = new unsigned char[6];
...
bool CDRATSServer::open()
{
	...
	m_future = std::async(std::launch::async, &CDRATSServer::Entry, this);
...
bool CDRATSServer::Entry()
{
	CUtils::lprint("Starting the D-RATS Server thread for %s", m_callsign.c_str());
	...
	std::chrono::high_resolution_clock::time_point hrct = std::chrono::high_resolution_clock::now();
	...
	try {
		while (!m_stopped) {
			...
			if (m_readEnd && !sending) {
				...
				hrct = std::chrono::high_resolution_clock::now();
			}

			if (m_readEnd && sending) {
				unsigned int dur = (unsigned int)std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - hrct).count();
				unsigned int needed = dur / DSTAR_FRAME_TIME_MS;
		...
		CUtils::lprint("Exception raised in the D-RATS Server thread - \"%s\"", message.c_str());
	}
	catch (...) {
		CUtils::lprint("Unknown exception raised in the D-RATS Server thread");
	}

	CUtils::lprint("Stopping the D-RATS Server thread for %s", m_callsign.c_str());
