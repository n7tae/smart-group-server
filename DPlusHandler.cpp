...
m_header(NULL)
{
	...
	char band = m_repeater.at(LONG_CALLSIGN_LENGTH - 1U);
...
void CDPlusHandler::process(const CPollData& poll)
{
	...
	CUtils::lprint("Incoming poll from unknown D-Plus dongle");
}
...
void CDPlusHandler::process(CConnectData& connect)
{
	...
	if (type != CT_LINK1) {
		CUtils::lprint("Incoming D-Plus message from unknown source");
		return;
	}
	...
	if (found) {
		...
		CUtils::lprint("No space to add new D-Plus dongle, ignoring");
		delete dplus;
	}
}
...
void CDPlusHandler::link(IReflectorCallback* handler, const std::string& repeater, const std::string &gateway, const in_addr& address)
{
	...
	if (found) {
		...
		CUtils::lprint("No space to add new D-Plus reflector, ignoring");
		delete dplus;
	}
}
		...
		if (reflector != NULL) {
			...
			if (exclude) {
				if (reflector->m_direction == DIR_OUTGOING && reflector->m_destination == handler && reflector->m_reflector.compare(callsign)) {
					CUtils::lprint("Removing outgoing D-Plus link %s, %s", reflector->m_repeater.c_str(), reflector->m_reflector.c_str());
				...
				if (reflector->m_destination == handler && 0 == reflector->m_reflector.compare(callsign)) {
					CUtils::lprint("Removing D-Plus link %s, %s", reflector->m_repeater.c_str(), reflector->m_reflector.c_str());
...
void CDPlusHandler::unlink()
{
	for (unsigned int i = 0U; i < m_maxReflectors; i++) {
		CDPlusHandler* reflector = m_reflectors[i];
		if (reflector != NULL) {
			if (reflector->m_reflector.size())
				CUtils::lprint("Unlinking from D-Plus reflector or dongle %s", reflector->m_reflector.c_str());
		...
		if (reflector != NULL) {
			if (reflector->m_reflector.size() && 0==reflector->m_reflector.compare(0, LONG_CALLSIGN_LENGTH-1, gatewayBase)) {
				if (address.size()) {
					...
					CUtils::lprint("IP address for D-Plus gateway or reflector %s has been removed", gatewayBase.c_str());
...
void CDPlusHandler::processInt(CHeaderData& header)
{
	...
	if (m_whiteList != NULL) {
		...
		if (!res) {
			CUtils::lprint("%s rejected from D-Plus as not found in the white list", my.c_str());
			m_dPlusId = 0x00U;
			return;
		}
	}
	...
	if (m_blackList != NULL) {
		...
		if (res) {
			CUtils::lprint("%s rejected from D-Plus as found in the black list", my.c_str());
			m_dPlusId = 0x00U;
			return;
		}
	}
	...
	switch (m_direction) {
		case DIR_OUTGOING:
			if (0==m_reflector.compare(rpt1) || 0==m_reflector.compare(rpt2)) {
			...
			switch (type) {
				case CT_ACK:
					if (m_linkState == DPLUS_LINKING) {
						CUtils::lprint("D-Plus ACK message received from %s", m_reflector.c_str());
				...
				case CT_NAK:
					if (m_linkState == DPLUS_LINKING) {
						CUtils::lprint("D-Plus NAK message received from %s", m_reflector.c_str());
				...
				case CT_UNLINK:
					if (m_linkState == DPLUS_UNLINKING) {
						CUtils::lprint("D-Plus disconnect acknowledgement received from %s", m_reflector.c_str());
		...
		case DIR_INCOMING:
			switch (type) {
				case CT_LINK2: {
						...
						CUtils::lprint("D-Plus dongle link to %s has started", m_reflector.c_str());
				...
				case CT_UNLINK:
					if (m_linkState == DPLUS_LINKED) {
						CUtils::lprint("D-Plus dongle link to %s has ended (unlinked)", m_reflector.c_str());
						m_stateChange = true;
...
bool CDPlusHandler::clockInt(unsigned int ms)
{
	...
	if (m_pollInactivityTimer.isRunning() && m_pollInactivityTimer.hasExpired()) {
		...
		if (m_reflector.size()) {
			switch (m_linkState) {
				case DPLUS_LINKING:
					CUtils::lprint("D-Plus link to %s has failed to connect", m_reflector.c_str());
					break;
				case DPLUS_LINKED:
					CUtils::lprint("D-Plus link to %s has failed (poll inactivity)", m_reflector.c_str());
					break;
				case DPLUS_UNLINKING:
					CUtils::lprint("D-Plus link to %s has failed to disconnect cleanly", m_reflector.c_str());
					break;
				default:
					break;
			}
...
void CDPlusHandler::writeStatus(FILE *file)
{
	for (unsigned int i = 0U; i < m_maxReflectors; i++) {
		CDPlusHandler* reflector = m_reflectors[i];
		if (reflector != NULL) {
			...
			if (reflector->m_linkState == DPLUS_LINKED) {
				switch (reflector->m_direction) {
					case DIR_OUTGOING:
						fprintf(file, "%04d-%02d-%02d %02d:%02d:%02d: DPlus link - Type: Dongle Rptr: %s Refl: %s Dir: Outgoing\n",
							tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec, 
							reflector->m_repeater.c_str(), reflector->m_reflector.c_str());
						break;

					case DIR_INCOMING:
						fprintf(file, "%04d-%02d-%02d %02d:%02d:%02d: DPlus link - Type: Dongle User: %s Dir: Incoming\n",
							tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec, 
