...
m_index(0U)
{
	...
	CUtils::lprint("Allocated UDP ports %u-%u to D-Plus", port, port + n - 1U);
}
...
CDPlusProtocolHandler* CDPlusProtocolHandlerPool::getHandler(unsigned int port)
{
	...
	CUtils::lprint("Cannot find a free D-Plus port in the pool");
...
void CDPlusProtocolHandlerPool::release(CDPlusProtocolHandler* handler)
{
	...
	CUtils::lprint("Trying to release an unused D-Plus port");
}
